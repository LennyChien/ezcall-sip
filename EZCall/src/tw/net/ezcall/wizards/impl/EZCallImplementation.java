...
import tw.net.ezcall.wizards.EZCallWizardIface;

import java.util.List;
import java.util.regex.Pattern;

public abstract class EZCallImplementation implements EZCallWizardIface {
	protected EZCallPrefsWizard parent;

	public void setParent(EZCallPrefsWizard aParent) {
		parent = aParent;
	}
	...
	//
	protected boolean isInputReady(EditText edt, int txtlength) {
		if (edt.getText() == null) {
			return false;
		}
		if (edt.getText().equals("")) {
	 ...
	 * 
	 * @param parentGroup
	 *            key for parent group if any. If null no parent group are
	 *            searched
	 * @param fieldName
	 *            key for the field to remove
	 */
	@SuppressWarnings("deprecation")
	protected void hidePreference(String parentGroup, String fieldName) {
		...
		if (toRemovePref != null && parentPref != null) {
			...
			Log.d("Generic prefs", "Not able to find" + parent + " "
					+ fieldName);
		}
	}
	...
	/**
	 * Set global preferences for this wizard If some preference that need
	 * restart are modified here Do not forget to return true in need restart
	 */
